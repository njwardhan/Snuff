...
The Snuff Core daemon can be configured to act as a trusted "border
router", implementing the snuff wire protocol and relay, making
...
The ZeroMQ feature in Snuff Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building snuffd:
...
For instance:
    ...
    $ snuffd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/snuffd.tx.raw \
...
These options can also be provided in snuff.conf.
...
From the perspective of snuffd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into snuffd directly. Furthermore, no information is
...
using. Snuffd appends an up-counting sequence number to each
