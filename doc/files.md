...
The data directory is the default location where the Snuff Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.snuff/`
macOS    | `$HOME/Library/Application Support/Snuff/`
Windows  | `%APPDATA%\Snuff\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `snuff.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Snuff blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `snuff.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `snuffd` or `snuff-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `snuffd.pid`        | Stores the process ID (PID) of `snuffd` or `snuff-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `snuffd` or `snuff-qt`; can be specified by `-debuglogfile` option
...
`snuff-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the Snuff Core:

Path           | Description | Repository notes
---------------|-------------|-----------------
`blktree/`     | Blockchain index; replaced by `blocks/index/` in [0.8.0](https://github.com/snuff-project/snuff/blob/master/doc/release-notes/release-notes-0.8.0.md#improvements) | [PR #2231](https://github.com/bitcoin/bitcoin/pull/2231), [`8fdc94cc`](https://github.com/snuff-project/snuff/commit/8fdc94cc8f0341e96b1edb3a5b56811c0b20bd15)
`coins/`       | Unspent transaction output database; replaced by `chainstate/` in 0.8.0 | [PR #2231](https://github.com/bitcoin/bitcoin/pull/2231), [`8fdc94cc`](https://github.com/snuff-project/snuff/commit/8fdc94cc8f0341e96b1edb3a5b56811c0b20bd15)
