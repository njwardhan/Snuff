Snuff Core version *0.15.1* is now available from:

  <https://download.snuff.org/snuff-0.15.1/>
...
A number of changes to the way Snuff Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 SNF). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 SNF) in the next release.
