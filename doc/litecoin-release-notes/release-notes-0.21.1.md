...
Summary:
==============
In this update to Snuff Core, Taproot has been activated, enhancing Snuff’s security, privacy and scalability. Snuff Core 0.21.1 is the most significant update in years - with performance improvements, new features (like Taproot), bug fixes and updated translations - and sets the foundations for MWEB’s imminent arrival. 

Snuff Core 0.21.1 is available now, right here: <https://download.snuff.org/snuff-0.21.1/>.

As always, we welcome the expertise of the community. To report bugs please use the issue tracker at GitHub - <https://github.com/snuff-project/snuff/issues > - and to get security and update notifications subscribe via <https://groups.google.com/forum/#!forum/snuff-dev>. 

Anyone can contribute to Snuff Core. Please scroll down to ‘How to contribute to Snuff Core’ for details.

By downloading or upgrading Snuff Core, you’re helping to secure the network, so thank you and enjoy the upgrade!


How to upgrade: 
==============

Firstly, thank you for running Snuff Core and helping secure the network!

As you’re running an older version of Snuff Core, shut it down. Wait until it’s completely shut down  - which might take a few minutes for older versions - then follow these simple steps:
For Windows: simply run the installer 
For Mac: copy over to `/Applications/Snuff-Qt` 
For Linux: copy cover `snuffd`/`snuff-qt`.

NB: upgrading directly from an ‘end of life’ version of Snuff Core is possible, but it might take a while if the data directory needs to be migrated. Old wallet versions of Snuff Core are generally supported.
 

Compatibility:
==============

Snuff Core is supported and extensively tested on operating systems using the Linux kernel, macOS 10.10+,  Windows 7 and newer. It’s not recommended to use Snuff Core on unsupported systems.

Snuff Core should also work on most other Unix-like systems, but is not as frequently tested on them.

From Snuff Core 0.21.1 onwards, macOS versions earlier than 10.12 are no longer supported. Additionally, Snuff Core doesn’t yet change appearance when macOS "dark mode" is activated.
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/Snuff/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Snuff Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Snuff Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/Snuff/bips/blob/master/bip-0157.mediawiki) compact
  filters to peers on the network when enabled using
  `-blockfilterindex=1 -peercfilters=1`. (#16442)

- This release implements
  [BIP339](https://github.com/Snuff/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/Snuff/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/Snuff/bips/blob/master/bip-0342.mediawiki)),
...
- A new `Snuff-cli -netinfo` command provides a network peer connections
...
- A new `Snuff-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `Snuff-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `Snuff-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Snuff Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `Snuff-wallet` tool which performs the salvage
...
Snuff Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`Snuff.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Snuff Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `Snuff-cli createwallet` or `Snuff-wallet
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Snuff Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `Snuff.conf`
