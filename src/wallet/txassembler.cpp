    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional SNF outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes SNF and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::SNF_TO_SNF) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a SNF-to-SNF transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::SNF_ONLY;
        ...
        // If SNF-to-SNF fails, create a peg-out transaction
