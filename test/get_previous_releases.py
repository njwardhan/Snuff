...
def download_binary(tag, args) -> int:
    if Path(tag).is_dir():
        ...
        shutil.rmtree(tag)
    Path(tag).mkdir()
    release_path = 'snuff-{}'.format(tag[1:])
    os = 'linux' # TODO
    tarball = 'snuff-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=args.platform)
    tarballUrl = 'https://download.snuff.org/{release_path}/{os}/{tarball}'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'snuff-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    githubUrl = "https://github.com/snuff-project/snuff"
    if args.remove_dir:
        if Path(tag).is_dir():
            shutil.rmtree(tag)
    ...
    with pushd(tag):
        ...
        files = ['snuffd', 'snuff-cli', 'snuff-tx']
        for f in files:
