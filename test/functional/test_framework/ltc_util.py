#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Snuff Core developers
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'snuff.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        bitcoind=os.path.join(bin_dir, 'snuffd'),
        bitcoin_cli=os.path.join(bin_dir, 'snuff-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
