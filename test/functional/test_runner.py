...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    try:
        ...
        if subprocess.run(["pgrep", "-x", "snuffd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a snuffd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `snuff-cli help` (`rpc_interface.txt`).
